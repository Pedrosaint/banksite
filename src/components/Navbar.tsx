...
export default function Navbar({ activeSection }: NavbarProps) {
  ...
  return (
    <nav className="fixed top-8 w-full bg-white/95 backdrop-blur-md z-50 border-b border-gray-200 transition-all duration-300">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Mobile Menu */}
        {isOpen && (
          <div className="md:hidden pb-4 space-y-2 animate-[fadeInUp_0.3s_ease-out]">
            {navItems.map((item) => (
              <button
                key={item.id}
                onClick={() => handleScroll(item.id)}
